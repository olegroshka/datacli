...
schema, one repair attempt on malformed JSON, everything cached and budgeted by
``llm.models.LLM``. Local (Ollama) by default; an API model is refused unless
    ...
    def _score_one(self, item: Item, schema: Schema) -> Result:
        ...
        for attempt in range(self.max_repairs + 1):
            ...
            if payload is not None:
                break
            # repair turn: show the model its own answer and ask for JSON only
            messages = messages + [
                {"role": "assistant", "content": raw[:4000]},
                {
                    "role": "user",
                    "content": "That was not a single valid JSON object. Return only "
                    "the JSON object in the required shape, with no prose.",
                },
            ]
        res.seconds = time.perf_counter() - t0
        res.raw = raw
