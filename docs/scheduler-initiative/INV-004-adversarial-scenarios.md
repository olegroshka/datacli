...
| AS-31 | HIGH | An unattended reboot (Windows Update at 02:00 on 2026-09-10) leaves no interactive session at trigger time. The `InteractiveToken` task cannot start, Windows treats it as a condition miss rather than a missed start, and `StartWhenAvailable` does not replay it after the next logon. No datacli run record exists. | Documented limitation of the default OQ-002 principal policy; the desired/backend/execution planes stay separate and nothing infers a run. Task Scheduler history (enabled 2026-09-11) gives Windows-side evidence. `TriggerSpec.logon = logged_off` (S4U, OQ-002 amendment 2026-09-11) removes the dependency on an interactive session; it is an explicit opt-in registered from an elevated terminal. | Logon/logoff timeline versus trigger time in the System log (Winlogon 7001/7002). OQ-002. |
