...
from fetch_eodhd_uk_eu_index_ref_universe import (
    INDEX_TICKERS_PATH,
    RAW_DIR,
    load_target_tickers,
)
...
def main() -> None:
    ...
    targets = load_target_tickers(
        explicit_specs=args.tickers, limit=args.limit, provider_path=INDEX_TICKERS_PATH
    )
    ...
    def _flush() -> None:
        ...
        if new_frames or new_state_rows:
            ...
            log.info(
                "  [FLUSH] UK/EU index-reference prices=%d rows (%d symbols)",
                rows,
                pairs,
            )
        new_frames.clear()
