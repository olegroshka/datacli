...
Usage:
    ...
    uv run python eodhd/fetch_eodhd_us_splits.py --tickers AAPL.US MSFT.US
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fetch EODHD US split history into the data root"
    )
    parser.add_argument(
    ...
    if state_lookup and not full_refresh:
        log.info(
            "Cached US split fetch-state rows on disk: %d firms", len(state_lookup)
        )
    ...
    def _flush() -> None:
        ...
        if non_empty_frames or new_audit_rows or new_state_rows:
            ...
            log.info(
                "  [FLUSH] US splits=%d rows (%d firms)", output_rows, output_firms
            )
        new_frames.clear()
