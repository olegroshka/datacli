...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fetch EODHD US ETF dividend history into the data root"
    )
    parser.add_argument(
    ...
    log.info(
        "US ETF dividend target count (%s universe): %d", args.universe, len(targets)
    )

    _, existing_audit, existing_output = load_completed_pairs(
        audit_path=DIVIDENDS_AUDIT_PATH, output_path=DIVIDENDS_PATH
    )
    existing_state = (
    ...
    for i, (ticker, exchange) in enumerate(targets):
        ...
        response = session.get(
            f"{EODHD_BASE}/div/{ticker}.{exchange}",
            params={
                "fmt": "json",
                "to": fetch_to,
