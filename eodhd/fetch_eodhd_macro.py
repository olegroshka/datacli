...
Usage:
    EODHD_API_KEY=xxx uv run python eodhd/fetch_eodhd_macro.py
...
def fetch_eodhd_macro(
    session: requests.Session, country: str, indicator: str
) -> pd.DataFrame:
    url = f"{EODHD_BASE}/macro-indicator/{country}"
    try:
        response = session.get(
            url, params={"indicator": indicator, "fmt": "json"}, timeout=HTTP_TIMEOUT
        )
    ...
    if response.status_code != 200:
        log.debug(
            "EODHD macro HTTP %s: %s/%s", response.status_code, country, indicator
        )
        return pd.DataFrame()
    try:
        data = response.json()
...
def main() -> None:
    ...
    for _, row in cov_df.iterrows():
        ...
        print(
            f"  {row['source']:<6} {row['actor_id']:<30} n={row['n_obs']:>5}  [{status}]"
        )
