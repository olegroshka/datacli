...
Usage:
    ...
    uv run python eodhd/fetch_eodhd_dividends.py --tickers SHEL.LSE SAP.XETRA
...
def normalize_dividend_rows(
    data: list[dict], ticker: str, exchange: str
) -> pd.DataFrame:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fetch EODHD dividend history into the data root"
    )
    parser.add_argument(
    ...
    if state_lookup and not full_refresh:
        log.info(
            "Cached dividend fetch-state rows on disk: %d firms", len(state_lookup)
        )
    ...
    def _flush() -> None:
        ...
        if non_empty_frames or new_audit_rows or new_state_rows:
            ...
            log.info(
                "  [FLUSH] dividends=%d rows (%d firms)", output_rows, output_firms
            )
        new_frames.clear()
